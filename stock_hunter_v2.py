...
def run():
    ...
    print(f"\nSTOCK HUNTER v2 - Early-Stage Screener")
    print(f"FROM_DATE (pick date): {from_date}")
    print(f"TO_DATE (evaluation date): {to_date}")
    print("-" * 75)
    ...
    for chunk_num, chunk in enumerate(chunk_list(symbols, CHUNK_SIZE), start=1):
        ...
        for sym_nse, sym_yf in zip(chunk, yf_tickers):
            try:
                ...
                result, reason = evaluate_stock(hist, from_date, to_date, sym_nse)
                if result:
    ...
    if not df_results.empty:
        df_results = df_results.sort_values(by="Forward_Return_%", ascending=False)
        df_results.to_csv("stock_hunter_v2_results.csv", index=False)
        print(f"\nQUALIFIED: {len(df_results)} stocks. Saved to stock_hunter_v2_results.csv")
    else:
        pd.DataFrame(columns=[
            "Stock", "Pick_Date", "Price_At_Pick", "Evaluation_Date", "Price_At_Evaluation",
        ...
        print("\nNo stocks qualified for this FROM_DATE. The filter is intentionally strict.")

    pd.DataFrame(skipped).to_csv("stock_hunter_v2_skipped.csv", index=False)
    print(f"Did not qualify / failed: {len(skipped)} (see stock_hunter_v2_skipped.csv for reasons)")
